...
case  24
    Cs=X;
    Fit = 100.*(Cs(:,1).^2 - Cs(:,2)).^2 + (Cs(:,1) - 1).^2 + (Cs(:,3) - 1).^2 + 90.*(Cs(:,3).^2 - Cs(:,4)).^2 + 10.1.*((Cs(:,2)-1).^2 + (Cs(:,4)-1).^2) + 19.8 .* (Cs(:,2)-1).*(Cs(:,4)-1);
...
case  26
    % keep values in the serach interval
        ...
        Fit= -(abs(sin(x1).*sin(x2).*exp(abs(100 - sqrt(x1.^2 + x2.^2)/pi))) + 1).^(-0.1);



case  27
    Fit = sum(abs(X), 2) .* exp(-sum(sin(X.^2), 2));
...
for i=1:dim-1
   ...
   p=p+((sin(((x(i)^2+x(i+1)^2)^0.5)-0.5)))^2/(0.001*(x(i)^2+x(i+1)^2)+1)^2+0.5; %sineenvlop
end
       Fit=-p;
